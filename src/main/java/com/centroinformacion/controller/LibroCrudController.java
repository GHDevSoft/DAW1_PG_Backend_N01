	...
	//LISTAR
		...
		public ResponseEntity<?> listaLibroPorTituloLike(@PathVariable("var") String titulo){
			...
			if (titulo.equals("todos")) {
				lstSalida = libroService.listaTodos();
			}else {
		...
		//REGISTRAR -- PARA IMPORTAR 8CONTROL+SHIF+O)
		...
		public ResponseEntity<?> insertaLibro(@RequestBody Libro obj) {
			...
			try {
				...
				DataCatalogo objEstado = new DataCatalogo();
	            objEstado.setIdDataCatalogo(27);
	            obj.setEstadoPrestamo(objEstado);
		...
		//ACTUALIZAR -- PARA IMPORTAR 8CONTROL+SHIF+O)
		...
		public ResponseEntity<Map<String, Object>> actualizaLibro(@RequestBody Libro obj) {
			...
			try {
				...
				if(!lstBusqueda.isEmpty()) {
					salida.put("mensaje", "La Libro " + obj.getTitulo() + " ya existe");
					return ResponseEntity.ok(salida);
				}
