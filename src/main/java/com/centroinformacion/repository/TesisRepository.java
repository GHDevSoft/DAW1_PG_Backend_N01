...
public interface TesisRepository extends JpaRepository<Tesis, Integer>{
	...
	public abstract List<Tesis> listaPorTituloLike(String titulo);
	
	@Query("select e from Tesis e where e.titulo = ?1 ")
	public abstract List<Tesis> listaPorTituloIgualRegistra(String titulo);
	
	@Query("select e from Tesis e where e.titulo = ?1 and e.idTesis != ?2 ")
	public abstract List<Tesis> listaPorTituloIgualActualiza(String titulo, int idTesis);		
